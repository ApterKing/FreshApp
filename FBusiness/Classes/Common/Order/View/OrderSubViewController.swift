...
class OrderSubViewController: BaseViewController {
    
    private lazy var tableView: UITableView = {
        let tv = UITableView.default
        tv.register(UINib(nibName: "OrderSubTableViewCell", bundle: Bundle(for: OrderSubTableViewCell.self)), forCellReuseIdentifier: NSStringFromClass(OrderSubTableViewCell.self))
...
extension OrderSubViewController {
    ...
    private func _initUI() {
        ...
        _ = NotificationCenter.default.rx.notification(Constants.notification_pay_success)
            .takeUntil(self.rx.deallocated)
            .subscribe(onNext: { [weak self] (_) in
                guard let weakSelf = self else { return }
                if weakSelf.status == .unpaid || weakSelf.status == .paid || weakSelf.status == .undeliver {
                    self?._loadData(true)
                }
            })
        
        _ = NotificationCenter.default.rx.notification(Constants.notification_order_canceled)
            .takeUntil(self.rx.deallocated)
            .subscribe(onNext: { [weak self] (_) in
                guard let weakSelf = self else { return }
                if weakSelf.status == .unpaid || weakSelf.status == .paid || weakSelf.status == .undeliver || weakSelf.status == .canceled {
                    self?._loadData(true)
                }
            })
...
extension OrderSubViewController: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: NSStringFromClass(OrderSubTableViewCell.self)) as? OrderSubTableViewCell {
...
extension OrderSubViewController: UITableViewDelegate {
    
    public func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return OrderSubTableViewCell.calculateHeight(with: vmodel.datas[indexPath.row])
    }
