//
//  HouseKeepingSubViewController.swift
//  FBusiness
//
//
...
class HouseKeepingSubViewController: BaseViewController {
    
    private lazy var tableView: UITableView = {
        let tv = UITableView.default
        tv.register(UINib(nibName: "HouseKeepingTableViewCell", bundle: Bundle(for: HouseKeepingTableViewCell.self)), forCellReuseIdentifier: NSStringFromClass(HouseKeepingTableViewCell.self))
    ...
    var status: HouseKeepingModel.Status = .unpaid
    private let vmodel = HouseKeepingVModel()
...
extension HouseKeepingSubViewController {
    ...
    private func _initUI() {
        ...
        _ = NotificationCenter.default.rx.notification(Constants.notification_pay_house_success)
            .takeUntil(self.rx.deallocated)
            .subscribe(onNext: { [weak self] (_) in
                guard let weakSelf = self else { return }
                if weakSelf.status == .unpaid || weakSelf.status == .paid {
                    self?._loadData(true)
                }
            })
        
        _ = NotificationCenter.default.rx.notification(Constants.notification_order_house_canceled)
            .takeUntil(self.rx.deallocated)
            .subscribe(onNext: { [weak self] (_) in
                guard let weakSelf = self else { return }
                if weakSelf.status != .finished {
                    self?._loadData(true)
                }
            })

        // 通知消息发送的订单状态改变
        _ = NotificationCenter.default.rx.notification(Constants.notification_order_home_status_changed)
            .takeUntil(self.rx.deallocated)
            .subscribe(onNext: { [weak self] (_) in
                self?._loadData(true)
            })
...
extension HouseKeepingSubViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: NSStringFromClass(HouseKeepingTableViewCell.self)) as? HouseKeepingTableViewCell {
...
extension HouseKeepingSubViewController: UITableViewDelegate {
    
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return HouseKeepingTableViewCell.calculateHeight(vmodel.datas[indexPath.row])
    }
