...
//  DFresh
//
//  Created by wangcong on 2019/3/28.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if UserVModel.default.isLogined {
            window?.rootViewController = XBaseNavigationController(rootViewController: DHomeViewController())
        } else {
            window?.rootViewController = XBaseNavigationController(rootViewController: DLoginViewController(nibName: "DLoginViewController", bundle: Bundle.currentDLogin))
        }
        window?.makeKeyAndVisible()
        
        DConfig.default.configAll(launchOptions: launchOptions)
        return true
    }
